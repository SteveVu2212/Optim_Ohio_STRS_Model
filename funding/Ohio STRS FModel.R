...
for(i in StartIndex:length(FYE)){
  ...
  CurrentHirePct[i] <- CurrentHirePct[i-1]*0.9
  ...
  DBPayroll_NewHires[i] <- NewHirePayroll[i]**(1 - (CombinedPlan_Enrolled_Pct + DC_Enrolled_Pct))
  DCPayroll_NewHires[i] <- NewHirePayroll[i]*DC_Enrolled_Pct
  CombinedPlanPayroll_NewHires[i] <- NewHirePayroll[i]*CombinedPlan_Enrolled_Pct
  ...
  #
  #EE NC, Contrib and Cost Sharing
  EmployeeNC_CurrentHires[i] <- (EEContribDB_CurrentHire*DBPayroll_CurrentHires[i] + EEContribCombined_DB_Transfer_CurrentHire*CombinedPlanPayroll_CurrentHires[i]) / 
    (DBPayroll_CurrentHires[i] + CombinedPlanPayroll_CurrentHires[i])
  
  EmployeeNC_NewHires[i] <- (EEContribDB_NewHire*DBPayroll_NewHires[i] + EEContribCombined_DB_Transfer_NewHire*CombinedPlanPayroll_NewHires[i]) / 
    (DBPayroll_NewHires[i] + CombinedPlanPayroll_NewHires[i])
  
  EmployerNC_CurrentHires[i] <- NC_DB_Pct_CurrentHire - EmployeeNC_CurrentHires[i]
  EmployerNC_NewHires[i] <- NC_DB_Pct_NewHire - EmployeeNC_NewHires[i]
  #
  #Amo Policy
  if(ER_Policy == 'Statutory Rate'){
    AmoRate_CurrentHires[i] <- (AmoRate_Stat_DB_CurrentHires*DBPayroll_CurrentHires[i] + AmoRate_Stat_Combined_CurrentHires*CombinedPlanPayroll_CurrentHires[i] + 
                                  ERContrib_DC_DB_UAL_CurrentHire*DCPayroll_CurrentHires[i] + ERContrib_DC_ARP_UAL_CurrentHire*ARPPayroll_CurrentHires[i]) / CurrentHirePayroll[i]
    
    AmoRate_NewHires[i] <- (AmoRate_Stat_DB_NewHires*DBPayroll_NewHires[i] + AmoRate_Stat_Combined_NewHires*CombinedPlanPayroll_NewHires[i] + 
                              ERContrib_DC_DB_UAL_NewHire*DCPayroll_NewHires[i] + ERContrib_DC_ARP_UAL_NewHire*ARPPayroll_NewHires[i]) / NewHirePayroll[i]
  ...
  #
  #EE and ER Cashflows
  EE_NC_CurrentHires[i] <- EmployeeNC_CurrentHires[i]*(DBPayroll_CurrentHires[i] + CombinedPlanPayroll_CurrentHires[i])
  EE_NC_NewHires[i] <- EmployeeNC_NewHires[i]*(DBPayroll_NewHires[i] + CombinedPlanPayroll_NewHires[i])
  ER_NC_CurrentHires[i] <- EmployerNC_CurrentHires[i]*(DBPayroll_CurrentHires[i] + CombinedPlanPayroll_CurrentHires[i])
  ER_NC_NewHires[i] <- EmployerNC_NewHires[i]*(DBPayroll_NewHires[i] + CombinedPlanPayroll_NewHires[i])
  ER_Amo_CurrentHires[i] <- max(AmoRate_CurrentHires[i]*TotalPayroll[i], -ER_NC_CurrentHires[i])
  ER_Amo_NewHires[i] <- max(AmoRate_NewHires[i]*NewHirePayroll[i], -ER_NC_NewHires[i])
  #
  ...
  Cashflows_Current <- BenPayments_CurrentHires[i] + AdminExp_CurrentHires[i] + EE_NC_CurrentHires[i] + ER_NC_CurrentHires[i] + ER_Amo_CurrentHires[i]
  Cashflows_New <- BenPayments_NewHires[i] + AdminExp_NewHires[i] + EE_NC_NewHires[i] + ER_NC_NewHires[i] + ER_Amo_NewHires[i]
  Cashflows_Total <- Cashflows_Current + Cashflows_New
  ...
  NetCF_CurrentHires[i] <- Cashflows_Current + Solv_Contrib_NewHires[i]
  ...
  NetCF_NewHires[i] <- Cashflows_New + Solv_Contrib_CurrentHires[i]
