...
##############################################################################################################################
IsRetirementEligible_Regular <- function(Age, YOS){
  Check = ifelse(Age >= 60 & YOS >= 5, TRUE, FALSE)
  
  return(Check)
}

IsRetirementEligible_Early <- function(Age, YOS){
  Check = ifelse(Age < 60 & YOS >= 5, TRUE, FALSE)
  return(Check)
}

IsRetirementEligible <- function(Age, YOS){
  Check = ifelse(IsRetirementEligible_Regular(Age, YOS) | IsRetirementEligible_Early(Age, YOS), TRUE, FALSE)
...
RetirementType <- function(Age, YOS){
  ...
  Check = ifelse(IsRetirementEligible_Regular(Age, YOS), 'Regular',
                 ifelse(IsRetirementEligible_Early(Age, YOS), 'Early','None'))
  
  return(Check)
}


SeparationType <- function(Age, YOS){
  Check = ifelse(IsRetirementEligible(Age, YOS) == T, 'Retirement',
...
MortalityTable <- MortalityTable_int %>% 
  ...
  mutate(RetirementCond = IsRetirementEligible(Age, YOS),
...
MortalityTable <- MortalityTable %>% select(EntryYear, term_year, entry_age, Age, YOS, mort)
...
SeparationRates <- SeparationRates %>% 
  mutate(retirement_type = IsRetirementEligible(Age,YOS),
...
SeparationRates <- SeparationRates %>% select(EntryYear, entry_age, Age, YOS, RemainingProb, SepProb, SepRate)

##############################################################################################################################

#Create a long-form table of Age and YOS and merge with salary data
SalaryData <- expand_grid(Age, YOS, Years) %>%  
  mutate(entry_age = Age - YOS,
         EntryYear = Years - YOS) %>%
...
SalaryData <- SalaryData %>%
  ...
  mutate(Salary = start_sal*cumprod(1+lag(salary_increase,default = 0))*(1 + payroll_growth)^(Years - YOS - YearStart),
         ...
         DB_EEContrib = DC_DB_EE_cont*Salary,
         DC_EEBalance = DC_EE_cont*Salary,
         DB_ERContrib = DB_ER_cont*Salary,
         DBERBalance = cumFV(0.03, (DB_EEContrib+DC_EEBalance)),
...
#          RF_After2015 = ifelse(RetType == "Early", EarlyRetPct_After2015/100/(YOS*BenMult_Hybrid),
#                                ifelse(RetType == "None", 0, 1))) %>% 
...
BenefitsTable <- AnnFactorData %>%
  ...
  mutate(Benefit = ifelse(RetirementType(Age,YOS) == 'Regular', DC_EEBalance + BenMult_Hybrid*FinalAvgSalary_5YR*YOS,
                ifelse(RetirementType(Age,YOS) == 'Early' & Age >= 50, DC_EEBalance + BenMult_Hybrid*FinalAvgSalary_5YR*YOS,
                       ifelse(RetirementType(Age,YOS) == 'Early', DC_EEBalance + BenMult_Hybrid*FinalAvgSalary_5YR*YOS, 0))),
         RF = 1,
         AnnFactorAdj = AnnuityFactor * surv_DR,
         PensionBenefit = RF * Benefit,
...
FinalData <- SalaryData %>% 
  ...
  left_join(SeparationRates, by = c("EntryYear", "Age", "YOS", "entry_age")) %>%
  mutate(SepType = SeparationType(Age,YOS),
         PenWealth = MaxBenefit,
